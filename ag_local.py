...
def detectStudentID(image, thresh):
    ...
    try:
        ...
        for c in contours:
            ...
            if (300 < w < 400) and (400 < h < 800) and (w/h<1) and check_rectangle(c):
...
def detectExamID(image, thresh):
    ...
    try:
        ...
        for c in contours:
            ...
            if (100 < w < 300) and (10 < h < 800) and (w/h<1) and check_rectangle(c):
                    thresh = thresh[y:y+h, x:x + w]
                    image = image[y:y + h, x:x + w]
                    break

        #thresh = cv2.erode(thresh, kernel=(15, 15),iterations=1)
        thresh = cv2.dilate(thresh, kernel=(5, 5), iterations=1)
        #thresh[thresh<255]= 0
        # cv2.imshow('b', thresh)
        # cv2.waitKey()
        contours = cv2.findContours(thresh, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
...
def detectResultSheet(image, thresh):
    ...
    for c in contours:
        ...
        if (100 < w < 500) and (200 < h < 500) and (w/h<=1.5):# and (y>image.shape[1]*1/3): #and check_points:
    ...
    for i in np.arange(0,len(fine_contours),6):
        ...
        for (j, c) in enumerate(cnts):
            ...
            ret_code, cellResult = detectResult(image[y:y+h,x:x+w],thresh[y:y+h,x:x+w])
            if ret_code!=0:
                ret_code = -1
                return ret_code, result
...
def mark():
    ...
    if one:
        glocal = True
        image = cv2.imread('data/Bai 120 cau  - new/P3 (19).jpg')
